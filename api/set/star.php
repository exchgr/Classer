<?
	...
	if ($star["subjAbbr"] == $c[0] && $star["code"] == $c[1]) {
		...
		echo "insert into stars (email, subjAbbr, code) values ('" . $email . "', '" . $c[0] . "', '" . $c[1] . "')"; // mysql_query("insert into stars (email, subjAbbr, code) values ('" . $email . "', '" . $star["subjAbbr"] . "', '" . $star["code"] . "')");
	}
